...
% Slide 12: QLoRA
\begin{frame}{QLoRA: 4-bit Quantized LoRA}
	\textbf{QLoRA} (Dettmers et al., 2023) combines 4-bit quantization with LoRA.

	\vspace{0.3cm}
	\textbf{How it works:}
	\begin{enumerate}
		\item \textbf{Quantize} base model to 4-bit NF4 (NormalFloat4)
		\item \textbf{Freeze} quantized weights --- no gradients through them
		\item \textbf{Add LoRA} adapters in FP16/BF16
		\item \textbf{Train} only LoRA parameters
	\end{enumerate}

	\vspace{0.3cm}
	\begin{block}{Memory Savings (Qwen2.5-0.5B)}
		\begin{itemize}
			\item FP16 base: $\sim$1.0 GB
			\item 4-bit NF4 base: $\sim$0.3 GB ($\mathbf{3.3\times}$ smaller)
			\item With double quantization: even more savings
		\end{itemize}
	\end{block}

	\vspace{0.2cm}
	\textbf{NF4 quantization:} Assumes weights follow a normal distribution; quantiles are optimally spaced for Gaussian data.
\end{frame}

% Slide 13: QLoRA Implementation
\begin{frame}[fragile]{QLoRA Implementation}
	\begin{lstlisting}[language=Python]
...
qlora_model = get_peft_model(base_model, lora_config)
	...
	\vspace{0.2cm}
	\textbf{Key insight:} LoRA config is identical regardless of quantization --- QLoRA is transparent to the user.
\end{frame}
