...
\subtitle{LLM07 Lecture}
\author{AUP Learning Cloud}
\date{}
...
\begin{frame}{The Problem: Attention is Memory-Bound}
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: GPU memory hierarchy diagram showing registers → SRAM → L2 → HBM]}}
\end{frame}
...
\begin{frame}{GPU Memory Hierarchy}
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: GPU architecture diagram showing SM, SRAM, HBM layout]}}
\end{frame}

% Slide 2b: Measuring Actual Bandwidth
\begin{frame}[fragile]{Measuring Actual Memory Bandwidth}
    \textbf{Lab Exercise:} Measure HBM bandwidth on your machine using tensor copy:
...
\begin{frame}{Standard Attention: Why $O(N^2)$ HBM Access?}
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: Memory flow diagram showing HBM reads/writes for standard attention]}}
\end{frame}

% Slide 4: Tiling for Matrix Multiplication
\begin{frame}{Tiling: Partitioning for SRAM}
    ...
    \vspace{0.3cm}
    Solution: \textbf{Online Softmax}

    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: Tiled matrix multiplication diagram showing tile loading]}}
\end{frame}
...
__global__ void tiledMatMul(float* A, float* B, float* C) {
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: HIP kernel execution flow showing shared memory usage]}}
\end{frame}
...
\begin{frame}{Softmax: Memory-Bound vs. Compute-Bound}
    \begin{table}
        \centering
        \small
        \begin{tabular}{llll}
...
\begin{frame}{Online Softmax: Single-Pass Solution}
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: Online softmax flow diagram showing incremental updates]}}
\end{frame}
...
\begin{frame}{FlashAttention V1: Core Algorithm}
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: FlashAttention V1 algorithm flow diagram]}}
\end{frame}
...
\begin{frame}{Benchmark: Standard vs Flash Attention}
    ...
    \vspace{0.3cm}
    {\footnotesize \textit{[INSERT IMAGE: Benchmark speedup chart showing speedup vs sequence length]}}
\end{frame}
...
\begin{frame}{Questions?}
    ...
    \begin{center}
        ...
        \url{https://amdresearch.github.io/aup-learning-cloud/}
    \end{center}
\end{frame}
