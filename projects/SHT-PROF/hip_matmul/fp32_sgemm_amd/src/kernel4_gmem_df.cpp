...
kernel4_gmem_db(float *a, float *b, float *c, int N, float alpha, float beta)
{
    // Block Tile size
...
void Kernel4GmemDB::init()
{
}
void Kernel4GmemDB::run(float *d_a, float *d_b, float *d_c, float alpha, float beta, int N)
{
    auto threadsPerBlock = dim3(BLOCK_SIZE);
    auto blocksPerGrid = dim3(N / 128, N / 128);
    hipLaunchKernelGGL(kernel4_gmem_db<1>, blocksPerGrid, threadsPerBlock, 0, 0, d_a, d_b, d_c, N, alpha, beta);
}

void Kernel4GmemDB::finalize()
{
}
